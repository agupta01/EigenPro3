...
class KernelModel:
    def __init__(
        ...
        n_nystrom_samples=5_000,
        data_preconditioner_level=500,
    ):
        ...
        if self.multi_gpu:
            ...
            self.weights_all = divide_to_gpus(
                self.weights, self.n_centers, self.devices
            )

            ######## dsitributing the centers over all avalibale GPUs
            self.centers_all = divide_to_gpus(
                self.centers, self.n_centers, self.devices
            )
        ...
        (
            self.data_preconditioner_matrix,
            self.eigenvectors_data,
            self.batch_size,
            self.lr,
        ...
        self.data_preconditioner_matrix = self.data_preconditioner_matrix.to(
            self.device_base
        )
    ...
    def fit(self, train_loaders, val_loader=None, epochs=10, score_fn=None):
        for t in range(epochs):
            ...
            if val_loader != None and score_fn != None:
                ...
                print(midrule)
                print(f"epoch {t+1:4d}        validation accuracy: {accu.item()*100.:5.2f}%")
                print(bottomrule)
    ...
    def get_gradient(self, X_batch_all, y_batch_all):
        ...
        with ThreadPoolExecutor() as executor:
            Kz_xbatch_chunk = [
                executor.submit(self.kernel, inputs[0], inputs[1])
                for inputs in zip(*[X_batch_all, self.centers_all])
            ]
        ...
        with ThreadPoolExecutor() as executor:
            gradients = [
                executor.submit(fmm, inputs[0], inputs[1], inputs[2], self.device_base)
                for inputs in zip(*[kxbatchz_all, self.weights_all, y_batch_all])
            ]
    ...
    def get_preconditioner(self, data_preconditioner_level):
        Lam_x, E_x, beta = nystrom_kernel_svd(
            self.nystrom_samples, self.kernel, data_preconditioner_level
        )
        ...
        E_x = E_x[:, : data_preconditioner_level - 1].cuda()
        D_x = ((1 - tail_eig_x / Lam_x) / Lam_x / nystrom_size).cuda()
        ...
        if batch_size < beta / tail_eig_x + 1:
            ...
            lr = (
                learning_rate_prefactor
                * batch_size
                / (beta + (batch_size - 1) * tail_eig_x)
            )

        print(
            f"Data: learning rate: {lr:.2f}, batch size={batch_size:5d}, top eigenvalue:{Lam_x[0]:.2f},"
            f" new top eigenvalue:{tail_eig_x:.2e}, beta:{beta:.2f}"
        )
        print("Data preconditioner is ready.")

        Kmat_xs_z = self.kernel(self.nystrom_samples, self.centers)
    ...
    def precondition_correction(self, X_batch, grad):
        ...
        preconditon_grad = self.data_preconditioner_matrix @ (
            self.eigenvectors_data.T @ (Kmat_xs_xbatch @ grad)
        )
        del Kmat_xs_xbatch
    ...
    def update_weights(self):
        ...
        new_weights = self.weights.to(self.device_base) - (
            self.lr / (self.batch_size)
        ) * self.theta2.to(self.device_base)
        ...
        if self.multi_gpu:
            for i in range(len(self.devices)):
                if i < len(self.devices) - 1:
                    ...
                    self.weights_all[i] = self.weights[
                        i * self.n_centers // len(self.devices) :, :
                    ].to(self.devices[i])
        else:
